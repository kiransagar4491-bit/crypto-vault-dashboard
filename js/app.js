...
function renderSignals() {
  ...
  const liveSignals = scannedSignals.length ? scannedSignals.map(scan => {
    ...
    const risk = Math.min(0.035, Math.max(0.004, move * 0.55));
    return { sym: scan.symbol, action: scan.action, entry: scan.price, target: scan.price * (1 + direction * move), stop: scan.price * (1 - direction * risk), strength: scan.confidence, indicator: scan.indicator, tf: '1h', generatedAt: scan.generatedAt, coin };
  }) : deriveLiveSignals();
...
let scanTimer = null;
let chartScans = {};
const SNIPER_CYCLE_STORAGE_KEY = 'cryptovault-sniper-cycles-v1';
let sniperSignals = [];
let sniperRequest = null;

function loadSniperCycles() {
  try { return JSON.parse(localStorage.getItem(SNIPER_CYCLE_STORAGE_KEY) || '{}'); } catch (_) { return {}; }
}
function saveSniperCycles(cycles) {
  try { localStorage.setItem(SNIPER_CYCLE_STORAGE_KEY, JSON.stringify(cycles)); } catch (_) {}
}
let sniperCycles = loadSniperCycles();

function sniperCycleHit(signal) {
  const coin = getCoin(signal.sym || signal.symbol);
  const price = coin?.price;
  if (!Number.isFinite(price) || !Number.isFinite(signal.target) || !Number.isFinite(signal.stop)) return false;
  return signal.action === 'sell'
    ? price <= signal.target || price >= signal.stop
    : price >= signal.target || price <= signal.stop;
}
function settleSniperCycles() {
  let changed = false;
  Object.keys(sniperCycles).forEach(sym => {
    if (sniperCycleHit(sniperCycles[sym])) { delete sniperCycles[sym]; changed = true; }
  });
  if (changed) saveSniperCycles(sniperCycles);
  sniperSignals = Object.values(sniperCycles).map(signal => ({ ...signal, coin: getCoin(signal.sym) }));
}
function applySniperCandidates(candidates) {
  settleSniperCycles();
  candidates.forEach(candidate => {
    if (!sniperCycles[candidate.sym]) sniperCycles[candidate.sym] = { ...candidate, cycleStartedAt: candidate.generatedAt || new Date().toISOString() };
  });
  saveSniperCycles(sniperCycles);
  sniperSignals = Object.values(sniperCycles).map(signal => ({ ...signal, coin: getCoin(signal.sym) }));
}
let tvChart = null;
...
async function fetchSniperSignals() {
  ...
  sniperRequest = fetch(SNIPER_SCAN_URL, { headers: { Accept: 'application/json' } })
    ...
    .then(payload => {
      const candidates = (payload.data || []).map(signal => ({
        ...signal,
        sym: signal.symbol,
      ...
      applySniperCandidates(candidates);
      renderSignals();
      return payload;
    })
    .catch(error => {
